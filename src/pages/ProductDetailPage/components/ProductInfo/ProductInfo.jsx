...
const ProductInfo = ({product, onOpenSizeQuiz}) => {
  ...
  useEffect(() => {
    if (!filteredSizes.some(s => s.size === selectedSize?.size)) setSelectedSize(null);
  }, [filteredSizes]);
  ...
  const cartItem = cart?.items?.find(item => item.variant === variant?.id);
  const stock = variant?.stock ?? 0;
  const isOutOfStock = !!variant && stock === 0;
  const limit = Math.min(MAX_PER_ITEM, stock);
  const currentQuantity = cartItem?.quantity ?? 0;
  const limitReached = !!variant && !isOutOfStock && currentQuantity >= limit;
